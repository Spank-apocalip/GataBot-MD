...
const smsAvisoAG = () => { return `╰⊱⚠️⊱ *𝙰𝚅𝙸𝚂𝙾* ⊱⚠️⊱╮\n\n` }
const smsAvisoIIG = () => { return `╰⊱❕⊱ *𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝙲̧𝙰̃𝙾* ⊱❕⊱╮\n\n` }
const smsAvisoFG = () => { return `╰⊱❌⊱ *𝙴𝚁𝚁𝙾𝚁* ⊱❌⊱╮\n\n` }
const smsAvisoMG = () => { return `╰⊱❗️⊱ *𝙰𝙲̧𝙰̃𝙾 𝙼𝙰𝙻 𝚄𝚂𝙰𝙳𝙰* ⊱❗️⊱╮\n\n` }
const smsAvisoEEG = () => { return `╰⊱📩⊱ *𝚁𝙴𝙿𝙾𝚁𝚃𝙴* ⊱📩⊱╮\n\n` }
const smsAvisoEG = () => { return `╰⊱💚⊱ *𝚂𝚄𝙲𝙴𝚂𝚂𝙾* ⊱💚⊱╮\n\n` }

//PARÁMETROS EN COMANDOS
const smsRowner = () => { return `\`\`\`¡¡𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾 𝙴𝚄 𝙲𝙾𝙼𝙾 𝙲𝚁𝙴𝙰𝙳𝙾𝚁(𝙰) 𝙳𝙾 𝙱𝙾𝚃 𝙿𝙾𝙳𝙴 𝚄𝚂𝙰𝚁!!\`\`\`` }//NUMERO DE BOT
const smsOwner = () => { return `\`\`\`¡¡𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾 𝙼𝙴𝚄 𝙲𝚁𝙸𝙰𝙳𝙾𝚁(𝙰) 𝙿𝙾𝙳𝙴 𝚄𝚂𝙰𝚁!!\`\`\`` }//OWNER
const smsMods = () => { return `\`\`\`¡¡𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾 𝙰𝙳𝙼𝙸𝙽𝚂 𝙾𝚄 𝙼𝙴𝚄 𝙲𝚁𝙸𝙰𝙳𝙾𝚁(𝙰) 𝙿𝙾𝙳𝙴 𝚄𝚂𝙰𝚁!!\`\`\`\`` }//MODERADORES
const smsPremium = () => { return `\`\`\`¡¡𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾 𝙴𝚂𝚃𝙰𝚁 𝙳𝙸𝚂𝙿𝙾𝙽𝙸́𝚅𝙴𝙻 𝙿𝙰𝚁𝙰 𝚄𝚂𝚄𝙰́𝚁𝙸𝙾𝚂 𝙿𝚁𝙴𝙼𝙸𝚄𝙼 𝙴 𝙼𝙴𝚄 𝙲𝚁𝙸𝙰𝙳𝙾𝚁(𝙰)!! 𝙿𝙰𝚁𝙰 𝚂𝙴𝚁 𝙿𝚁𝙴𝙼𝙸𝚄𝙼 𝙲𝙾𝙼𝙿𝚁𝙴 𝚄𝙼 𝙿𝙰𝚂𝙴 𝚄𝚂𝙰𝙽𝙳𝙾 #pass premium\`\`\`` }//USUARIOS PREMIUM
const smsGroup = () => { return `\`\`\`¡¡𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾 𝚂𝙴𝚁 𝚄𝚃𝙸𝙻𝙸𝚉𝙰𝙳𝙾 𝙴𝙼 𝙶𝚁𝚄𝙿𝙾𝚂!!\`\`\`` }//PARA GRUPOS
const smsPrivate = () => { return `\`\`\`¡¡𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾 𝙿𝙾𝙳𝙴 𝚂𝙴𝚁 𝚄𝚃𝙸𝙻𝙸𝚉𝙰𝙳𝙾 𝙽𝙾 𝙿𝚁𝙸𝚅𝙰𝙳𝙾!!\`\`\`` }//AL PRIVADO
const smsAdmin = () => { return `\`\`\`¡¡𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾 𝙴 𝙿𝙰𝚁𝙰 𝙰𝙳𝙼𝙸𝙽𝚂!!\`\`\`` }//ADMINS
const smsBotAdmin = () => { return `\`\`\`¡¡𝙿𝚁𝙴𝙲𝙸𝚂𝙾 𝚂𝙴𝚁 𝙰𝙳𝙼𝙸𝙽 𝙿𝙰𝚁𝙰 𝙿𝙾𝙳𝙴𝚁 𝚄𝚂𝙰𝚁 𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾!!\`\`\`` }//BOT CON ADMIN
const smsUnreg = () => { return `\`\`\`¡¡𝙿𝚁𝙴𝙲𝙸𝚂𝙰 𝙴𝚂𝚃𝙰𝚁 𝚁𝙴𝙶𝙸𝚂𝚃𝚁𝙰𝙳𝙾(𝙰) 𝙿𝙰𝚁𝙰 𝚄𝚂𝙰𝚁 𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾, 𝙴𝚂𝙲𝚁𝙴𝚅𝙴 #verificar 𝙿𝙰𝚁𝙰 𝚂𝙴 𝚁𝙴𝙶𝙸𝚂𝚃𝚁𝙰𝚁!!\`\`\`` }//VERIFICAR
const smsRestrict = () => { return `\`\`\`¡¡𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝙴𝚂𝚃𝙰𝚁 𝚁𝙴𝚂𝚃𝚁𝙸𝙽𝙶𝙸𝙽𝙳𝙾 𝙿𝙴𝙻𝙾 𝙼𝙴𝚄 𝙲𝚁𝙸𝙰𝙳𝙾𝚁(𝙰)!!\`\`\`` }//RESTRINGIR COMANDO

//MENU LISTA
const smsTime = () => { return `𝚃𝙴𝙼𝙿𝙾 𝙰𝚃𝚄𝙰𝙻`}
const smsUptime = () => { return `𝙵𝚄𝙽𝙲𝙸𝙾𝙽𝙰𝙽𝙳𝙾 𝙳𝚄𝚁𝙰𝙽𝚃𝙴`}
const smsVersion = () => { return `𝚅𝙴𝚁𝚂𝙰̃𝙾 𝙳𝙴 ${global.packname}`}
const smsTotalUsers = () => { return `𝚃𝙾𝚃𝙰𝙻 𝙳𝙴 𝚄𝚂𝚄𝙰́𝚁𝙸𝙾𝚂`}
const smsMode = () => { return `𝙴𝚂𝚃𝙰𝚁 𝙴𝙼 𝙼𝙾𝙳𝙾`}
const smsModePublic = () => { return `𝙿𝚄𝙱𝙻𝙸𝙲𝙾`}
const smsModePrivate = () => { return `𝙿𝚁𝙸𝙱𝙰𝙳𝙾`}
const smsBanChats = () => { return `𝙲𝙷𝙰𝚃(𝚂) 𝙿𝚁𝙾𝙸𝙱𝙸𝙳𝙾(𝚂)`}
const smsBanUsers = () => { return `𝚄𝚂𝚄𝙰𝚁𝙸𝙾𝚂(𝚂) 𝙿𝚁𝙾𝙸𝙱𝙸𝙳𝙾(𝚂)`}
const smsPareja = () => { return `𝙿𝙰𝙴𝙲𝙴𝙸𝚁𝙾`}
const smsResultPareja = () => { return `𝙿𝙰𝚁𝙲𝙴𝙸𝚁𝙾(𝙰)`}
const smsSaludo = () => { return `👋 𝙷𝙾𝙻𝙰 𝙱𝙴𝙼𝚅𝙸𝙽𝙳𝙾(𝙰) 👋`}
const smsDia = () => { return `🌇 𝙱𝙾𝙼 𝙳𝙸𝙰 ⛅`}
const smsTarde = () => { return `🏙️ 𝙱𝙾𝙰 𝚃𝙰𝚁𝙳𝙴 🌤️`}
const smsTarde2 = () => { return `🌆 𝙱𝙾𝙰 𝚃𝙰𝚁𝙳𝙴 🌥️`}
const smsNoche = () => { return `🌃 𝙱𝙾𝙰 𝙽𝙾𝙸𝚃𝙴 💫`}
const smsListaMenu = () => { return `⊹ 𝙻𝙸𝚂𝚃𝙰 𝙳𝙴 𝙼𝙴𝙽𝚄 ⊹`}
const smsLista1 = () => { return `🌟 𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝙲̧𝙾̃𝙴𝚂 𝙳𝙴 𝙳𝙰𝚁𝚃𝙷 𝙼𝙳 🌟`}
const smsLista2 = () => { return `🗿 𝙲𝚁𝙸𝙰𝙳𝙾𝚁 🗿`}
const smsLista3 = () => { return `🎁 𝙳𝙾𝙽𝙰𝚁 🎁`}
const smsLista4 = () => { return `🚀 𝚅𝙴𝙻𝙾𝙲𝙸𝙳𝙰𝙳𝙴 🚀`}
const smsLista5 = () => { return `💡 𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝙲̧𝙰̃𝙾 𝙳𝙴 𝙼𝙴𝙽𝚄 💡`}
const smsLista6 = () => { return `🌀 𝙼𝙴𝙽𝚄 𝙲𝙾𝙽𝙿𝙻𝙴𝚃𝙾 🌀`}
const smsLista7 = () => { return ` 𝙸𝙽𝚂𝚃𝙰𝙻𝙰𝙲̧𝙰̃𝙾 𝙳𝙴 𝙳𝙰𝚁𝚃𝙷𝙱𝙾𝚃 `}
const smsLista8 = () => { return `🍄 𝚂𝙴𝚁 𝚂𝚄𝙱 𝙱𝙾𝚃 🍄`}
const smsLista9 = () => { return `📄 𝚃𝙴𝚁𝙼𝙾𝚂, 𝙲𝙾𝙽𝙳𝙸𝙲̧𝙾̃𝙴𝚂 𝙴 𝙿𝚁𝙸𝚅𝙰𝙲𝙸𝙳𝙰𝙳𝙴 📄`}
const smsLista10 = () => { return `🌟 𝙰𝚅𝙴𝙽𝚃𝚄𝚁𝙰 🌟`}
const smsLista11 = () => { return `🏆 𝚃𝙾𝙿 𝙶𝙻𝙾𝙱𝙰𝙻 🏆`}
const smsLista12 = () => { return `🏅 𝚄𝚂𝚄𝙰́𝚁𝙸𝙾𝚂 𝙿𝚁𝙴𝙼𝙸𝚄𝙼 🏅`}
const smsLista13 = () => { return `🎟️ 𝚂𝙴𝚁 𝚄𝚂𝚄𝙰𝚁𝙸𝙾(𝙰) 𝙿𝚁𝙴𝙼𝙸𝚄𝙼 🎟️`}
const smsLista14 = () => { return `🛣️ 𝙼𝙸𝚂𝚂𝙾̃𝙴𝚂 𝙳𝙸𝙰́𝚁𝙸𝙰𝚂 🛣️`}
const smsLista15 = () => { return `⚗️ 𝙼𝙴𝙽𝚄 𝚁𝙿𝙶 ⚗️`}
const smsLista16 = () => { return `🏪 𝚃𝙴𝙽𝙳𝙰 𝙳𝙴 𝙲𝙾𝙼𝙿𝚁𝙰 𝙴 𝚅𝙴𝙽𝙳𝙰 🏪`}
const smsLista17 = () => { return `🎒 𝙸𝙽𝚅𝙴𝙽𝚃𝙰́𝚁𝙸𝙾 🎒`}
const smsLista18 = () => { return `🌟 𝙼𝚄𝙸𝙻𝚃𝙼𝙴𝙳𝙸𝙰 🌟`}
const smsLista19 = () => { return `📲 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙳𝙾𝚆𝙽𝙻𝙾𝙰𝙳𝚂 📲`}
const smsLista20 = () => { return `🔍 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙿𝙴𝚂𝚀𝚄𝙸𝚂𝙰𝚂 🔍`}
const smsLista21 = () => { return `🛰️ 𝙼𝙴𝙽𝚄 𝙲𝙾𝙽𝚅𝙴𝚁𝚃𝙸𝙳𝙾𝚁 🛰️`}
const smsLista22 = () => { return `🧰 𝙼𝙴𝙽𝚄 𝙼𝙾𝙳𝙸𝙵𝙸𝙲𝙰𝙳𝙾𝚁 𝙳𝙴 𝙰𝚄𝙳𝙸𝙾 🧰`}
const smsLista22_1 = () => { return `🔩 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙵𝙴𝚁𝚁𝙰𝙼𝙴𝙽𝚃𝙰𝚂 🔩`}
const smsLista23 = () => { return `🌟 𝙳𝙸𝚅𝙴𝚁𝚂𝙰̃𝙾 🌟`}
const smsLista24 = () => { return `🎡 𝙹𝙾𝙶𝙾𝚂 𝙳𝙸𝙽𝙰̂𝙼𝙸𝙲𝙾𝚂 🎡`}
const smsLista25 = () => { return `🔊 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙰𝚄𝙳𝙸𝙾𝚂 🔊`}
const smsLista26 = () => { return `🎈 𝙼𝙴𝙽𝚄 𝙳𝙴 𝚂𝚃𝙸𝙲𝙺𝙴𝚂 𝙴 𝙵𝙸𝙻𝚃𝚁𝙾𝚂 🎈`}
const smsLista27 = () => { return `✨ 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙴𝙵𝙴𝙸𝚃𝙾𝚂 𝙴 𝙻𝙾𝙶𝙾𝚂 ✨`}
const smsLista28 = () => { return `🌅 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙻𝙾𝙶𝙾𝚂 2 🌅`}
const smsLista29 = () => { return `⛩️ 𝙼𝙴𝙼𝙴𝚂 𝚁𝙰𝙽𝙳𝙾𝙼𝚂 : 𝙰𝙽𝙸𝙼𝙴 ⛩️`}
const smsLista30 = () => { return `🔞 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾𝚂 +18 🔞`}
const smsLista31 = () => { return `🌟 𝙰𝙹𝚄𝚂𝚃𝙴𝚂 🌟`}
const smsLista32 = () => { return `🔰 𝙼𝙴𝙽𝚄 𝙿𝙰𝚁𝙰 𝙶𝚁𝚄𝙿𝙾𝚂 🔰`}
const smsLista33 = () => { return `📑 𝙻𝙸𝚂𝚃𝙰𝚂 𝙳𝙸𝚂𝙿𝙾𝙽𝙸́𝚅𝙴𝙸𝚂 📑`}
const smsLista34 = () => { return `⚙️ 𝙲𝙴𝙽𝚃𝚁𝙾 𝙳𝙴 𝙲𝙾𝙽𝙵𝙸𝙶𝚄𝚁𝙰𝙲̧𝙾̃𝙴𝚂 ⚙️`}
const smsLista35 = () => { return `💎 𝙼𝙴𝙽𝚄 𝙿𝙰𝚁𝙰 𝚆𝙾𝙽𝙴𝚁 💎`}

//main.js
const smsWelcome = () => { return `*╭┈⊰* @subject *⊰┈ ✦*\n*┊✨ 𝙱𝙴𝙽𝚅𝙸𝙽𝙳𝙾 𝙲𝙰𝙻𝙰𝙱𝚁𝙴𝚂𝙾(𝙰)!!*\n┊😏 @user\n┊📄 *𝙻𝙴𝙸𝙰 𝙰 𝙳𝙴𝚂𝙲𝚁𝙸𝙲̧𝙰̃𝙾 𝙳𝙾 𝙲𝙷𝙰𝚃 𝚂𝙴*\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✦*\n${String.fromCharCode(8206).repeat(850)}\n@desc`}
const smsBye = () => { return '*╭┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*\n┊ @user\n┊ *𝚂𝙰𝙸𝚄 𝙼𝙰𝙸𝚂 𝚄𝙼 𝙲𝙾𝚁𝙽𝙾 𝙳𝙾 𝙲𝙷𝙰𝚃, 𝙵𝙳𝚂!!* 🏌️‍♂️\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*'}
const smsSpromote = () => { return '*@user 𝙰𝙶𝙾𝚁𝙰 𝙴 𝙰𝙳𝙼𝙸𝙽 𝙳𝙾 𝙲𝙷𝙰𝚃 𝙶𝙰𝚁𝙰𝙸!!*'}
const smsSdemote = () => { return '*@user 𝙳𝙴𝙸𝚇𝙰 𝙳𝙴 𝚂𝙴𝚁 𝙰𝙳𝙼𝙸𝙽 𝚅𝙸𝚁𝙾𝚄 𝙱𝚁𝙾𝚇𝙰 𝙳𝙾 𝙲𝙷𝙰𝚃!!*'}
const smsSdesc = () => { return '*𝙰 𝙽𝙾𝚅𝙰 𝙳𝙴𝚂𝙲𝚁𝙸𝙲̧𝙰̃𝙾 𝙳𝙾 𝙶𝚁𝚄𝙿𝙾 𝙴:*\n\n@desc'}
const smsSsubject = () => { return '*𝙾 𝙽𝙾𝚅𝙾 𝙽𝙾𝙼𝙴 𝙳𝙾 𝙶𝚁𝚄𝙿𝙾 𝙴:*\n\n@subject'}
const smsSicon = () => { return '*𝙰𝙲𝙰𝙱𝙰𝚁𝙰𝙼 𝙳𝙴 𝚃𝚁𝙾𝙲𝙰 𝙰 𝙵𝙾𝚃𝙾 𝙳𝙾 𝙶𝚁𝚄𝙿𝙾!!*'}
...
//_allantilink.js
const smsTextoYT = () => { return '🥷 𝙱𝙸𝙶 𝙼𝙲 𝙾𝙵𝙲'} 
const smsApagar = () => { return '❌ 𝙳𝙴𝚂𝙰𝚃𝙸𝚅𝙰𝙳𝙾'} 
const smsEncender = () => { return '✅ 𝙰𝚃𝙸𝚅𝙰𝙳𝙾'} 
...
const smsCont9 = () => { return `${global.lenguajeGB['smsAvisoAG']()}*𝙿𝚁𝙴𝙲𝙸𝚂𝙰 𝙳𝙾 𝙽𝙸𝚅𝙴𝙻 ➡️*`}
const smsCont10 = () => { return `*𝙿𝙰𝚁𝙰 𝚄𝚂𝙰𝚁 𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾. 𝚃𝙴𝚄 𝙽𝙸𝚅𝙴𝙻 𝙰𝚃𝚄𝙰𝙻 𝙴 ➡️*`}
...
const smsInt2 = () => { return `𝙽𝙰́𝙾 𝙴𝚂𝚃𝙰́ 𝙿𝙴𝚁𝙼𝙸𝚃𝙸𝙳𝙾 𝙽𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾!!`}

//_antilink.js
const smsAdwa = () => { return `${global.lenguajeGB['smsAvisoEG']()}𝙲𝙾𝙼𝙾 𝙴 𝙰𝙳𝙼𝙸𝙽 𝙳𝙾 𝙶𝚁𝚄𝙿𝙾 𝙽𝙰̃𝙾 𝚂𝙴𝚁𝙰 𝙱𝙰𝙽𝙸𝙳𝙾(𝘼)`}
...
//Texto
const smsConMenu = () => { return `☘️ 𝗠𝗘𝗡𝗨`} //🟡 NO CAMBIAR 
...
//_antitoxic.js
const smsToxic1 = () => { return `𝙽𝙰̃𝙾!!! 🤬 𝙳𝙸𝙶𝙰 𝙴𝚂𝚃𝙰 𝙿𝙰𝙻𝙰𝚅𝚁𝙰 𝙾𝚃𝙰𝚁𝙸𝙾`}
const smsToxic2 = () => { return `𝙴𝚂𝚃𝙰 𝙿𝚁𝙾𝙸𝙱𝙸𝙳𝙾 𝙽𝙰́𝙾 𝚂𝙴𝙹𝙰 𝚃𝙾𝚇𝙸𝙲𝙾(𝙰)`}
const smsToxic3 = () => { return `*𝙰𝚅𝙸𝚂𝙾*\n⚠️`}
...
const smsToxic7 = () => { return `𝚂𝚄𝙿𝙴𝚁𝙾𝚄 𝙰𝚂 4 𝙰𝙳𝚅𝙴𝚁𝚃𝙴̂𝙽𝙲𝙸𝙰𝚂 𝙰𝙶𝙾𝚁𝙰 𝚂𝙴𝚁𝙰 𝙴𝙻𝙸𝙼𝙸𝙽𝙰𝙳𝙾(𝙰) 🙄`}

//Tienda
const eExp = () => { return '⚡ 𝙴𝚡𝚙𝚎𝚛𝚒𝚎̂𝚗𝚌𝚒𝚊' } 
const eDiamante = () => { return '💎 𝙳𝚒𝚊𝚖𝚊𝚗𝚝𝚎𝚜' } 
const eDiamantePlus = () => { return '💎+ 𝙳𝚒𝚊𝚖𝚊𝚗𝚝𝚎+' }
const eToken = () => { return '🪙 𝚃𝚘𝚔𝚎𝚗' } 
const eEsmeralda = () => { return '💚 𝙴𝚜𝚖𝚎𝚛𝚊𝚕𝚍𝚊' } 
const eJoya = () => { return '♦️ 𝙹𝚘𝚢𝚊' }
const eMagia = () => { return '🌀 𝙼𝚊𝚐𝚒𝚊' } 
const eOro = () => { return '👑 Oro' } 
const eGataCoins = () => { return '🗿 𝙳𝙰𝚁𝙺𝙲𝙾𝙸𝙽𝚃𝚂' }
const eGataTickers = () => { return '🎫 𝙳𝙰𝚁𝚃𝙷 𝚃𝙸𝙲𝙺𝙴𝚁𝚂' } 
const eEnergia = () => { return '✨ 𝙴𝚗𝚎𝚛𝚐𝚒𝚊' }
const ePocion = () => { return '🥤 𝙿𝚘́𝚌𝚒𝚘𝚗' }
const eAgua = () => { return '💧 𝙰́𝚐𝚞𝚊' }
...
//config-on y off.js
const smsWel1 = () => { return `*𝙵𝚄𝙽𝙲-𝚆𝙴𝙻𝙲𝙾𝙼𝙴*`}
const smsWel2 = () => { return `mensagem de bemvindo para novos membros em Grupos*`}
const smsDete1 = () => { return `*𝙵𝚄𝙽𝙲-𝙰𝚅𝙸𝚂𝙾𝚂*`}
const smsDete2 = () => { return `*avisos de ação dentro do grupo*`}
const smsANivel1 = () => { return `*𝙽𝙸𝚅𝙴𝙻-𝙰𝚄𝚃𝙾𝙼𝙰𝚃𝙸𝙲𝙾*`}
const smsANivel2 = () => { return `*sube de nivel de manera automática*; (*aplica recompensas por subir de Nivel*)`}
const smsRestri1 = () => { return `*𝙵𝚄𝙽-𝚁𝙴𝚂𝚃𝚁𝙸𝙽𝙶𝙸𝚁*`}
const smsRestri2 = () => { return `*habilitar função para adicionar o banir pessoas em grupos*`}
const smsLlamar1 = () => { return `*𝙰𝙽𝚃𝙸-𝙲𝙷𝙰𝙼𝙰𝙳𝙰𝚂*`}
const smsLlamar2 = () => { return `*bloquea as pessoas que me fazer chamdas*`}
const smsAntiSp1 = () => { return `*𝙰𝙽𝚃𝙸-𝚂𝙿𝙰𝙼*`}
const smsAntiSp2 = () => { return `*banir o uso de comados quando alguém fazer algum tipo de spam*`}
const smsModP1 = () => { return `*𝙼𝙾𝙳𝙾-𝙿𝚄𝙱𝙻𝙸𝙲𝙾*`}
const smsModP2 = () => { return `*habilitar função para que todos possam usar darth-md*`}
const smsModAd1 = () => { return `*𝙼𝙾𝙳𝙾-𝙰𝙳𝙼𝙸𝙽*`}
const smsModAd2 = () => { return `*so os admins poders usar darth-md em grupos*`}
const smsLect1 = () => { return `*𝙻𝙴𝙸𝚃𝚄𝚁𝙰-𝙰𝚄𝚃𝙾𝙼𝙰𝚃𝙸𝙲𝙰*`}
const smsLect2 = () => { return `*deixar mensagens ou bate-papos como lidas*`}
const smsTempo1 = () => { return `*𝙱𝙾𝚃-𝚃𝙴𝙼𝙿𝙾𝚁𝙰𝙻*`}
const smsTempo2 = () => { return `*função que permite permanecer temporariamente em grupos*`}                                 
const smsStik1 = () => { return `*𝙵𝚄𝙽𝙲-𝚂𝚃𝙸𝙲𝙺𝙴𝚁𝚂*`}
const smsStik2 = () => { return `*habilitar o envio automático de stickers a todos*`}
const smsStickA1 = () => { return `*𝙰𝚄𝚃𝙾-𝚂𝚃𝙸𝙲𝙺𝙴𝚁𝚂*`}
const smsStickA2 = () => { return `*vídeos, gif, imagens, links jpg ou jpeg*; *Eles se tornarão adesivos automaticamente*`}
const smsReacc1 = () => { return `*𝙵𝚄𝙽𝙲-𝚁𝙴𝙰𝙶𝙸𝚁*`}
const smsReacc2 = () => { return `*habilite o envio automático de reações às mensagens*`}
const smsAudi1 = () => { return `*𝙰𝚄𝚃𝙾-𝙰𝚄𝙳𝙸𝙾𝚂*`}
const smsAudi2 = () => { return `*habilitar el envio automático de Audios a todos*`}
const smsModHor1 = () => { return `*𝙼𝙾𝙳𝙾-𝚇𝚅𝙸𝙳𝙴𝙾𝚂*`}
...
const smsAntiEli1 = () => { return `*𝙰𝙽𝚃𝙸-𝙳𝙴𝙻𝙴𝚃*`}
const smsAntiEli2 = () => { return `*Todas as mensagens excluídas serão encaminhadas para o Chat ou Grupo*`}
const smsAntiInt1 = () => { return `*𝙰𝙽𝚃𝙸-𝙵𝙰𝙺𝙴*`}
const smsAntiInt2 = () => { return `*remover números internacionais considerados falsos*`}
const smsAntiE1 = () => { return `*𝙰𝙽𝚃𝙸-𝙻𝙸𝙽𝙺𝚂*`}
const smsAntiE2 = () => { return `*excluir pessoas que enviam links de grupos do WhatsApp*`}
const smsAntiEE1 = () => { return `*𝙰𝙽𝚃𝙸-𝙻𝙸𝙽𝙺𝚂2*`}
...
const smsAntiTEL1 = () => { return `*𝙰𝙽𝚃𝙸-𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙽*`}
...
const smsMens2 = () => { return `𝙰𝚃𝚄𝙰𝙻𝙼𝙴𝙽𝚃𝙴`} 
const smsMens3 = () => { return `𝙽𝙴𝚂𝚃𝙴`} 
const smsMens4 = () => { return `𝙱𝙾𝚃`} 
const smsMens5 = () => { return `𝙲𝙷𝙰𝚃`} 

//Error2
const smsMensError1 = () => { return `❕ REPORTAR COMANDO ❕`} 
const smsMensError2 = () => { return `Está faltando o siguiente comando`} 

//_antiviewonce.js
const smsAntiView = () => { return `*𝙈𝘼𝙎𝙏𝙀𝙍 𝙔𝙊 𝙇𝙊 𝙑𝙀𝙊 𝙏𝙊𝘿𝙊 𝘼𝙌𝙐Í* 😎`} 

//_autolevelup.js
const smsAutoLv1 = () => { return `🎖️ NOVO NIVEL 🎖️`} 
...
const smsGrupoTime7 = () => { return '🔒 *O GRUPO ESTA FECHADO, SOMENTES ADMINS PODEN ENVIAR MENSAGENS*'}
const smsGrupoTime8 = () => { return '🔓 *O GRUPO ESTA ABERTO, TODOS PODEN ENVIAR MENSAGENS*'}
...
const smsPropban5 = () => { return `${lenguajeGB['smsAvisoEG']()}𝙾 𝚄𝚂𝚄𝙰́𝚁𝙸𝙾(𝙰) 𝙵𝙾𝙸 𝙱𝙰𝙽𝙸𝙳𝙾(𝙰) \n𝙴𝚂𝚃𝙰𝚁 𝙿𝚁𝙾𝙸𝙱𝙸𝙳𝙾 𝙳𝙴 𝚄𝚂𝙰𝚁  ${gt}`}
...
const smsContacto8 = () => { return 'Esta Conta e um Bot 👀'}
